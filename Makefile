...
sources/%.heatmap.bz2: sources/%.snapshot.bz2
	bzcat $< | parse-bgp -c $(patsubst sources/%.snapshot.bz2,heatmap-data/%.config.json,$<) | bzip2 -c > $@

heatmap-data/%.annotations: heatmap-data/%.annotations.in
	v6-to-fakev4 -c $(patsubst heatmap-data/%.annotations.in,heatmap-data/%.config.json,$<) < $< >$@

heatmap-data/%.shade: heatmap-data/%.shade.in
	v6-to-fakev4 -c $(patsubst heatmap-data/%.shade.in,heatmap-data/%.config.json,$<) < $< >$@
